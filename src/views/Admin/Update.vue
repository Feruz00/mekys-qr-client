...
import { useCreateUser, useGetUser, useUpdateUser } from './useUsers';
import { computed, onMounted, ref, watch } from 'vue';

const topBar = [
  { name: 'admin', title: 'Ulanyjylar' },
  { name: 'edit-user', title: 'Ulanyjy maglumatlaryny sazlamak' },
];
...
const { isLoading, isError, isSuccess, data, error } = useGetUser(id);
...
const { isPending, mutate, isSuccess: isSuccessUpdate } = useUpdateUser();
...
const reloadData = () => {
  if (data.value?.data) {
    ...
    form.value = {
      ...
      qr_limit: val.qr_limit,
      status: val.status,
    };
  }
};
watch(data, reloadData);

onMounted(() => {
  document.title = 'Admin | Ulanyjy  maglumatyny sazlamak';
  reloadData();
});
</script>
